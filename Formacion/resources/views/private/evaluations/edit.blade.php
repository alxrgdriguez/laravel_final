        ...
        <div class="max-w-2xl w-full bg-white dark:bg-gray-800 shadow-lg rounded-xl p-8 border border-gray-300 dark:border-gray-700">
            ...
            <form action="{{ route('admin.evaluations.update', $evaluation->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" readonly value="{{ $evaluation->user->name }} {{ $evaluation->user->surnames }}"
                ...
                <div>
                    ...
                    <input type="text" readonly value="{{ $evaluation->course->name }}"
                           class="w-full px-4 py-3 bg-gray-200 dark:bg-gray-700 dark:text-gray-200 rounded-lg cursor-not-allowed">
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.1" min="0" max="10" id="final_note" name="final_note"
                           value="{{ old('final_note', $evaluation->final_note) }}"
                ...
                <div>
                    ...
                    <textarea id="comments" name="comments" rows="4"
                              class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-4 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">{{ old('comments', $evaluation->comments) }}</textarea>
                    @error('comments')
                    <span class="text-sm text-red-600">{{ $message }}</span>
                    @enderror
                </div>
